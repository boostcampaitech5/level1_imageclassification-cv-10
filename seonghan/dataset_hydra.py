...
class AlbumAugmentation:
    def __init__(self, resize, mean, std, centercrop, **args):
        self.transform = A.Compose([
            A.CenterCrop(centercrop[0], centercrop[1]),
            A.Resize(resize[0], resize[1], p=1.0, interpolation=cv2.INTER_LANCZOS4),
...
class MaskMultiDataset(Dataset):
    num_classes = 3 * 2 * 3
    ...
    def __getitem__(self, index):
        ...
        age_label = self.get_age_label(index)
        # multi_class_label = self.encode_multi_class(mask_label, gender_label, age_label)
        multi_class_label = np.array([mask_label, gender_label, age_label])
