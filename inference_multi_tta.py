...
def inference(data_dir, model_dir, output_dir, args):
    ...
    for model_name in model_list: 
        ...
        with torch.no_grad():
            for idx, images in enumerate(loader):
                ...
                tmp_mask = (mask_outs+mask_flip_outs+mask_cj_outs)/3
                tmp_age = (age_outs+age_flip_outs+age_cj_outs)/3
            ...
            fold_mask = np.array(mask_preds)
            fold_age = np.array(age_preds)
            fold_gender = np.array(gender_preds)

        if idx==0:
            oof_mask = (fold_mask)/len(model_list)
            oof_age = (fold_age)/len(model_list)
            oof_gender = (gender_outs+gender_flip_outs+gender_cj_outs)/len(model_list)
        else:
            oof_mask += (fold_mask)/len(model_list)
            oof_age += (fold_age)/len(model_list)
    ...
    gender_pred = torch.where(oof_gender <= torch.tensor(0.5), torch.tensor([0]).to(device), torch.tensor([1]).to(device)).squeeze()
