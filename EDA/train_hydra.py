...
#                 return mask_label * 6 + gender_label * 3 + age_label
            ...
            if (idx + 1) % args.log_interval == 0 or idx + 1 == len(train_loader):
                ...
                mask_train_acc = mask_matches / args.log_interval
                gender_train_loss = gender_loss_value / args.log_interval
                gender_train_acc = gender_matches / args.log_interval
                age_train_loss = age_loss_value / args.log_interval
                age_train_acc = age_matches / args.log_interval
                ...
                print(
                    ...
                    f"{'mask training':>15} {args.criterion} loss {mask_train_loss:4.4}\t|| training {args.evaluation} {mask_train_acc:4.2%}\t|| lr {current_lr}"
                )
                print(
                    f"Epoch[{epoch+1}/{args.epochs}] ({idx + 1}/{len(train_loader)})\t|| "
                    f"{'gender training':>15} {args.criterion} loss {gender_train_loss:4.4}\t|| training {args.evaluation} {gender_train_acc:4.2%}\t|| lr {current_lr}"
                )
                print(
                    f"Epoch[{epoch+1}/{args.epochs}] ({idx + 1}/{len(train_loader)})\t|| "
                    f"{'age training':>15} {args.criterion} loss {age_train_loss:4.4}\t|| training {args.evaluation} {age_train_acc:4.2%}\t|| lr {current_lr}"
                )
                print(
                    f"Epoch[{epoch+1}/{args.epochs}] ({idx + 1}/{len(train_loader)})\t|| "
...
#                 preds = torch.argmax(outs, dim=-1)  # torch.Tensor([0, 1, 2, 0, 0, 0, 2, 0... ])
    ...
    #                 return mask_label * 6 + gender_label * 3 + age_label
                ...
                evaluation_item = eval_dict[args.evaluation](preds.data.cpu(), labels.data.cpu()).item()
...
#                 evaluation_item = eval_dict[args.evaluation](preds.data.cpu(), labels.data.cpu())
            ...
            mask_val_acc = np.sum(mask_val_acc_items) / len(val_evaluation_items)
            gender_val_loss = np.sum(gender_val_loss_items) / len(val_loader)
            gender_val_acc = np.sum(gender_val_acc_items) / len(val_evaluation_items)
            age_val_loss = np.sum(age_val_loss_items) / len(val_loader)
            age_val_acc = np.sum(age_val_acc_items) / len(val_evaluation_items)
            ...
            if counter > patience:
                ...
                break
            print(
                f"[Val] mask {args.evaluation} : {mask_val_acc:4.2%}, {args.criterion}: {mask_val_loss:4.2}\t|| "
                f"best {args.evaluation} : {mask_best_val_acc:4.2%}, best {args.criterion}: {mask_best_val_loss:4.2}"
            )
            print(
                f"[Val] gender {args.evaluation} : {gender_val_acc:4.2%}, {args.criterion}: {gender_val_loss:4.2}\t|| "
                f"best {args.evaluation} : {gender_best_val_acc:4.2%}, best {args.criterion}: {gender_best_val_loss:4.2}"
            )
            print(
                f"[Val] age {args.evaluation} : {age_val_acc:4.2%}, {args.criterion}: {age_val_loss:4.2}\t|| "
                f"best {args.evaluation} : {age_best_val_acc:4.2%}, best {args.criterion}: {age_best_val_loss:4.2}"
            )
            print(
